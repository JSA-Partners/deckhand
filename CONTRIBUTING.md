...
Every change on `main` that users install is released: the version is bumped in `pyproject.toml`,
`deckhand/__init__.py`, and both manifests in one `chore(release): X.Y.Z` commit, tagged `vX.Y.Z`.
Semver follows the conventional type of the change: `fix` is a patch, `feat` a minor, and a breaking
change a major.
