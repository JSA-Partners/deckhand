...
sends each story to its repository, recording which way each dependency runs. A parked feature
becomes the story it stood for, keeping its number, so everything already pointing at it points at
the work.
