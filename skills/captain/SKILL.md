...
step, never touch a branch, never commit. A row whose fix names a command is the person's to run in
the session that owns that story, so name it and leave it.

Answer in plain sentences, not tables: the tables are above and they have been read.
